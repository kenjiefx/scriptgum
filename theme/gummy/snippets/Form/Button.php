...
$class = 'background-color-'.$button['type'].' '.$button['class'].' color-'.$text_color;
?>
...
<button class="text-3 cursor-pointer border-style-none padding-x-7 padding-y-5 border-radius-extra-small-5 display-flex align-items-center <?php echo $class; ?>" <?php if (null!==$button['xclick']) echo 'xclick="'.$button['xclick'].'"'; ?>>
    <?php 
    if ($button['icon']!==null) snippet('Icon/SVG',['path'=>$button['icon'],'class'=>'small-width-4 margin-right-4 '.$icon_color,'icon_stroke_width'=>$button['icon_stroke_width']])
    ?>
